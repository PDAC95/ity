...
const SOCIAL_ICONS: Record<string, React.ReactNode> = {
  ...
  x: (
    <svg viewBox="0 0 24 24" className="h-4 w-4" fill="currentColor">
      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.737-8.846L1.254 2.25H8.08l4.259 5.63 5.905-5.63zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
    </svg>
  ),
  youtube: (
    <svg viewBox="0 0 24 24" className="h-4 w-4" fill="currentColor">
...
export function ProfilePreview({ school, avatarUrl }: ProfilePreviewProps) {
  ...
  const activeSocialLinks = socialLinks
    ? Object.entries(socialLinks).filter(([, val]) => val && val.trim() !== '')
    : [];
        ...
        {mode === 'profile' ? (
          ...
          <div className="rounded-lg border border-zinc-800 bg-zinc-900 overflow-hidden">
            <div
              ...
              style={{
                backgroundColor: school?.branding?.primaryColor ?? '#6366F1',
              }}
            >
              {/* School name / logo */}
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-white/90">{name || 'Tú'}</span>
              </div>
            </div>
          </div>
        )}
