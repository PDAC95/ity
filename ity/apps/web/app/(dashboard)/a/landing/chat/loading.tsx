export default function Loading() {
  return (
    <div className="flex flex-col h-full animate-pulse">
      ...
      <div className="flex items-center justify-center gap-2 py-3 border-b border-zinc-800">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="h-2 w-8 rounded-full bg-zinc-800" />
        ))}
      </div>
      ...
      <div className="flex-1 space-y-4 p-4 overflow-hidden">
        ...
        <div className="flex justify-end">
          <div className="h-10 w-48 rounded-2xl bg-zinc-800" />
        </div>
        {/* AI message - left */}
        <div className="flex gap-3">
          <div className="h-8 w-8 rounded-full bg-zinc-800 flex-shrink-0" />
          <div className="h-20 w-72 rounded-2xl bg-zinc-800" />
        </div>
        {/* User message - right */}
        <div className="flex justify-end">
          <div className="h-12 w-56 rounded-2xl bg-zinc-800" />
        </div>
      </div>
